...
class Board():
  ...
  def __init__(self):
    self.width = 8
    self.height = 8
    self.squares = [[0 for _ in range(self.width)] for _ in range(self.height)]

  def coordConv(self):
    letters = "ABCDEFGH"
    self.coords = [[0 for _ in range(8)] for _ in range(8)] #initialisierung eines zweidimensionalen 8x8 arrays gefüllt mit 0
    i = 8
